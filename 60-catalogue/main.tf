...
resource "aws_ec2_instance_state" "catalogue" {
  ...
  state       = "stopped"
  depends_on = [ terraform_data.catalogue ]  # once resource "terraform_data" "catalogue" is completed with sucess then only resource "aws_ec2_instance_state" "catalogue" will start   
}
...
resource "aws_autoscaling_group" "catalogue" {
  ...
  max_size           = 10
  min_size           = 1
  target_group_arns = [aws_lb_target_group.catalogue.arn] #arns Amazon Resource Name
